...
export const Calculator = () => {
  const [startedTestFood, setStartedTestFood] = useState(false);
  const [startedTestTransport, setStartedTestTransport] = useState(false);
  ...
  return (
    <Stack mr="70px" ml="70px" mt="100px" mb="70px">
      {!startedTestFood && !startedTestTransport && (
        <>
          ...
          <Stack>
            ...
            <Button
              ...
              onClick={() => setStartedTestFood(true)}
            >
              Commencer le test
            </Button>
          </Stack>
      ...
      {foodValid && !startedTestTransport && (
        <Stack mr="70px" ml="70px" mb="70px">
          <Heading size="xl" mb="20px">
            <HStack>
              <Text>C'est fini pour l'</Text>{" "}
              <Text color="#FD8D25">alimentation</Text>
              <Text>, on passe au</Text> <Text color="#6B70D6">transport</Text>{" "}
            </HStack>
          </Heading>
          <Text fontWeight={"semibold"} mb="20px">
            Tu as déjà fait la moitié du chemin ! Si tu veux t'arrêter ici car
            tu dois y aller tu peux, ta progression est sauvegardé
            automatiquement pour pouvoir reprendre un test que tu as commencé,
            ou même refaire un test pour comparer avec tes anciens résultats.
          </Text>
          <Button
            backgroundColor={"#00CC81"}
            color={"white"}
            ...
            w="400px"
            _hover={{ backgroundColor: "#78E2BB" }}
            onClick={() => setStartedTestTransport(true)}
          >
            On commence la partie transport{" "}
          </Button>
        </Stack>
      )}
      {startedTestFood && !foodValid && (
        <FormFood setFoodValid={setFoodValid} />
      )}
      {startedTestTransport && !transportValid && (
        <FormDriver setTransportValid={setTransportValid} />
      )}
    </Stack>
  );
};
